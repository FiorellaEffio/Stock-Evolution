  ...
  <v-container class="text-xs-center">
    ...
    <v-flex mt-5>
      <v-btn flat large @click="changeFactor(0)" slot="activator" class="btnEco">
        <div>
          <h5>Lo define la entidad financiera<br> intermediaria de la bolsa de valores
          </h5>
        </div>
      </v-btn>
      <v-btn flat large @click="changeFactor(1)" slot="activator" class="btnEco">
        <div>
          <h5>Lo define un cáculo en el que influyen<br> factores propios de la empresa</h5>
        </div>
      </v-btn>
      <v-btn flat large @click="changeFactor(2)" slot="activator" class="btnEco">
        <div>
          <h4>Lo define la demanda
          </h4>
        </div>
      </v-btn>
    </v-flex>
...
export default {
  ...
  methods: {
    nextLevelTwo() {
      if (this.factorOption === 1) {
        EventBus.$emit("change-leoFive2", true);
      } else {
        this.snackbar = true;
      }
    },
